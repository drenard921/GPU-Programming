...
//
// Provides:
//   1. CPU baseline particle update path for comparison with CUDA
//   2. Bag, line, and chamber state updates for simplified protocol simulation
//   3. Rotea-inspired particle motion using normalized force-balance behavior
//   4. Shared initialization logic for fluid-like and cell-like particles
//
// Notes:
//   - This is a physically motivated visualization model, not a calibrated
//     Rotea process model.
//   - The CPU path is used both for visual CPU mode and for headless CPU
//     benchmarking.
//   - Particle motion approximates local flow, Stokes-like drag,
//     density-dependent centrifugal migration, and conical chamber constraints.
// =============================================================================
...
// Initializes fluid-like and cell-like particles inside the conical annulus.
// Fluid particles are lower-density tracers; cell particles are larger and
// denser so they respond more strongly to centrifugal retention.
std::vector<Particle> initParticles(
    ...
    auto sampleInShell = [&](int type) {
        ...
        float rr = std::sqrt(
            u01(rng) * (rOuter * rOuter - rInner * rInner) +
            rInner * rInner
        );
        ...
        if (type == 1) {
            ...
            // Fluid tracer / background particle.
            p.diameter = 0.06f;
            p.density  = 1.00f;
        }
...
// Updates the simplified process-level state: bag volumes, line flow,
// chamber suspended/retained cells, and chamber rotational strength.
//
// This model is intentionally coarse. It provides protocol context for the
// visual simulation and benchmark runs, while the per-particle force behavior
// is handled separately in updateParticles().
void updateSystem(
    std::vector<Bag>& bags,
    ...
    if (chamber.media_density <= 0.0f) {
        chamber.media_density = 1.00f;
    }

    if (chamber.media_viscosity <= 0.0f) {
        chamber.media_viscosity = 1.00f;
    }
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        if (!sourceIsChamber) {
            srcPressure = bags[line.sourceBag].pressure_kpa;
        }

        if (!targetIsChamber) {
            dstPressure = bags[line.targetBag].pressure_kpa;
        }

        float dP = srcPressure - dstPressure;

        float effectiveFlow =
            commandedFlow * clampf(1.0f + 0.01f * dP, 0.25f, 2.0f);
        ...
        // CHAMBER -> BAG
            ...
            float suspendedOut =
                std::min(chamber.suspended_cells, volumeTransfer * 0.08f);

            chamber.suspended_cells -= suspendedOut;

            if (step.phase == PhaseType::Harvest) {
                float retainedOut =
                    std::min(chamber.retained_cells, volumeTransfer * 0.15f);
    ...
    // This approximates the process-level idea that higher G-force promotes
    // retention, while stronger flow promotes washout/elutriation. It is separate
    // from the particle-level visualization model below.
...
// Enforces the simplified CFC chamber geometry.
// Particles are wrapped vertically and clamped between the inner and outer
// cone radii so the visual state remains inside the annular chamber.
...
// CPU particle update loop.
//
// Each particle is advanced using a normalized Rotea-inspired model:
//   - local tip-driven flow and wall return flow
//   - tangential swirl from chamber rotation
//   - Stokes-like drag toward local fluid velocity
//   - density- and diameter-dependent centrifugal migration
//   - cone-boundary constraints and numerical guards
//
// This CPU path serves as the serial baseline for CUDA speedup comparisons.
void updateParticles(
    ...
    // Later, chamber.media_density / chamber.media_viscosity could be routed here.
    const float mediaDensity   = 1.00f;
    const float mediaViscosity = 1.00f;

    // Convert commanded G-force into normalized rotational strength.
    // This is a visualization scaling, not a physical SI-unit conversion.
    ...
    for (auto& p : particles) {
        ...
        float radialNozzleWeight =
            std::exp(-(r * r) / std::max(jetRadius * jetRadius, eps));

        float jetFalloff =
            std::exp(-jetDecay * distToTip) * radialNozzleWeight;
        ...
        float wallFrac = (r - rInner) / gap;
        wallFrac = clampf(wallFrac, 0.0f, 1.0f);

        float nearOuter =
            clampf((wallFrac - 0.65f) / 0.35f, 0.0f, 1.0f);

        float vReturnY =
            -wallReturnStrength * flow * 0.0025f * nearOuter;

        float vReturnX =
            -rx * wallReturnStrength * flow * 0.0015f * nearOuter;

        float vReturnZ =
            -rz * wallReturnStrength * flow * 0.0015f * nearOuter;
        ...
        // 5. Density-dependent centrifugal tendency.
